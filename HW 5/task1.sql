...
FROM restBill
WHERE cust_name = 'Bob Crow'; -- Filters to include only records where the customer name is 'Bob Crow'

-- 2
SELECT DISTINCT cust_name -- Selects unique customer names
FROM restBill
WHERE cust_name LIKE '% Smith' -- Filters customers whose names end with 'Smith'
ORDER BY cust_name DESC; -- Orders the result by 'cust_name' in descending alphabetical order

-- 3
SELECT DISTINCT cust_name -- Selects unique customer names
FROM restBill
WHERE cust_name LIKE '% C%'; -- Filters customers whose names contain the letter 'C'

-- 4
SELECT DISTINCT CONCAT(first_name, ' ', surname) AS headwaiter_name -- Concatenates first and surname into a single 'headwaiter_name' column
FROM restStaff
WHERE headwaiter IS NULL; -- Filters to only include records where the 'headwaiter' field is NULL (meaning these staff members are not headwaiters)

-- 5
SELECT *
FROM restBill
WHERE bill_date BETWEEN 160201 AND 160229; -- Filters bills that occurred between February 1, 2016 and February 29, 2016 (format: YYMMDD)

-- 6
SELECT DISTINCT bill_date -- Selects unique bill dates
FROM restBill
WHERE bill_date BETWEEN 150101 AND 150931 -- Filters for bill dates between January 1, 2015, and September 30, 2015 (format: YYMMDD)
ORDER BY bill_date; -- Orders the result by 'bill_date' in ascending order
