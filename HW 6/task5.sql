...
SELECT 
    ...
    restBill.bill_date,
    COUNT(restBill.bill_no) AS num_bills
FROM restBill
...
HAVING COUNT(restBill.bill_no) >= 2;  -- Filters for waiters who have taken 2 or more bills on the same date

-- 2
SELECT 
    restRest_table.room_name,
    COUNT(restRest_table.table_no) AS num_tables
FROM restRest_table
WHERE restRest_table.no_of_seats > 6  -- Filters for tables with more than 6 seats
...
FROM restBill
INNER JOIN restRest_table ON restBill.table_no = restRest_table.table_no  -- Joins to get table details
INNER JOIN restRoom_management ON restRest_table.room_name = restRoom_management.room_name  -- Joins to get room details
...
FROM restBill
INNER JOIN restStaff ON restBill.waiter_no = restStaff.staff_no  -- Joins with restStaff to get waiter details
INNER JOIN restRoom_management ON restRoom_management.headwaiter = restStaff.staff_no  -- Joins with restRoom_management to get headwaiter details
GROUP BY restRoom_management.headwaiter
ORDER BY total_bill_amount DESC;  -- Orders by total bill amount, largest first
...
HAVING AVG(restBill.bill_total) > 400.00;  -- Filters for customers who spent more than £400 on average
...
INNER JOIN restStaff ON restBill.waiter_no = restStaff.staff_no  -- Joins with restStaff to get waiter details
GROUP BY restBill.waiter_no, restBill.bill_date
HAVING COUNT(restBill.bill_no) >= 3;  -- Filters for waiters who have taken 3 or more bills on the same date
